...
describe CsvFile do
  ...
  it 'should mark a valid csv as valid' do
    csv_importer = CsvFile.new(Rails.root.join('spec', 'files', 'valid_csv_example.csv'), ',', 'utf-8', 1)
    csv_importer.is_valid?.should be_true
  end
  
  it 'should mark an invalid csv as invalid' do
    csv_importer = CsvFile.new(Rails.root.join('spec', 'files', 'header.xls'), ';', 'utf-8', 1)
    csv_importer.is_valid?.should be_false
  end
  
  it 'should read the header' do
    csv_importer = CsvFile.new(Rails.root.join('spec', 'files', 'valid_csv_example.csv'), ',', 'utf-8', 1, false, true)
