...
describe Etl::LawyerExtraction do
  ...
  describe 'parsing' do
    ...
    it 'should get information from a document' do
      VCR.use_cassette('advokat_295627') do
        ...
        document = extractor.download
        lawyer = extractor.digest(document)
        lawyer.should == {:original_name=>"ADAMČÍK Andrej Mgr.", :first_name => 'Andrej', :last_name => 'Adamčík', :title => 'Mgr.', :lawyer_type=>"Advokát", :street=>"Sedlárska 1", :city=>"Bratislava 1", :zip=>"811 01", :phone=>"02/3219 1153", :fax=>"02/3214 4888", :cell_phone=>"0917/611153", :languages=>"anglicky", :email=>"andrej.adamcik@hillbridges.com", :website=>nil, :url => "https://www.sak.sk/blox/cms/sk/sak/adv/vyhladanie/proxy/list/formular/rows/295627/link/display/event", :sak_id => 295627, is_part_of_import:true}
      end
    end
  ...
  it 'should update a lawyer if it is already in the database' do
    VCR.use_cassette('advokat_4690') do
