  ...
  describe 'parsing' do
    ...
    it 'should get information from a document' do
      VCR.use_cassette('advokat_295627') do
        ...
        document = extractor.download
        lawyer = extractor.digest(document)
        lawyer.should == {:original_name=>"ADAMČÍK Andrej Mgr.", :first_name => 'Andrej', :last_name => 'ADAMČÍK', :title => 'Mgr.', :lawyer_type=>"Advokát", :street=>"Sedlárska 1", :city=>"Bratislava 1", :zip=>"811 01", :phone=>"02/3219 1153", :fax=>"02/3214 4888", :cell_phone=>"0917/611153", :languages=>"anglicky", :email=>"andrej.adamcik@hillbridges.com", :website=>nil, :url => "https://www.sak.sk/blox/cms/sk/sak/adv/vyhladanie/proxy/list/formular/rows/295627/link/display/event", :sak_id => 295627, :ds_lawyer_associates_attributes => [], is_part_of_import:true}
      end
    end
    ...
    it 'should get information from a document' do
      VCR.use_cassette('advokat_4515') do
        ...
        document = extractor.download
        lawyer = extractor.digest(document)
        lawyer.should == {:original_name=>"ADAMČÍK Rudolf JUDr.", :first_name => 'Rudolf', :last_name => 'ADAMČÍK', :title => 'JUDr.', :lawyer_type=>"Advokát", :street=>"Liptovská 2/A, P.O. BOX 67", :city=>"BRATISLAVA 2", :zip=>"820 05", :phone=>"02/53413792, 53412855", :fax=>"02/53413793", :cell_phone=>"", :languages=>"nemecky, rusky", :email=>"office@ada-ap.sk", :website=>nil, :url=>"https://www.sak.sk/blox/cms/sk/sak/adv/vyhladanie/proxy/list/formular/rows/4515/link/display/event", :sak_id => 4515, :ds_lawyer_associates_attributes=>[{:first_name=>"Čuraj", :last_name=>"Lukáš", :title=>"Mgr.", :original_name => 'Čuraj Lukáš Mgr.', is_part_of_import:true}, {:first_name=>"Štiak", :last_name=>"Peter", :title=>"MUDr.Mgr.", :original_name => 'Štiak Peter MUDr.Mgr.', is_part_of_import:true}], is_part_of_import:true}
      end
    end
    ...
    it 'should not save the same record with a different url twice' do
      VCR.use_cassette('advokat_167897') do
        ...
        Kernel::DsLawyer.count.should == 1
      end
    end
